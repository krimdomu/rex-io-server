...
sub get_set {
    ...
    if ( !$set ) {
        return $self->render( json => { ok => Mojo::JSON->false },
            status => 404 );
    }

    $self->render(
        json => { ok => Mojo::JSON->true, data => $set->to_hashRef } );
}
...
sub add_perm {
    ...
    eval {
        my $perm =
          $self->db->resultset('Permission')->create( $self->req->json );
...
sub list_permission_types {
    ...
    try {
        ...
        return $self->render(
            json => { ok => Mojo::JSON->true, data => \@ret } );
    }
    catch {
        return $self->render(
