...
#my $hw = Rex::IO::Server::Model::Hardware->all( Rex::IO::Server::Model::NetworkAdapter->mac == \@mac_addresses );
        ...
        if ( !$hw_o ) {
          ...
          eval {
            ...
            $redis_deploy->publish(
              $self->config->{redis}->{deploy}->{queue} => Mojo::JSON->encode(
                {
                  cmd  => "deploy",
                  type => "newsystem",
          ...
          && $self->config->{deploy}->{run_services_after_install} )
        {
          ...
          for my $task ( $hw_o->tasks ) {
            ...
            $client->job_servers(
              @{ $self->config->{gearman}->{job_servers} } );
            my $arg = {
              service => [
                {
                  service => $task->task->service->service_name,
      ...
      # return action
        ...
        for my $client ( keys %{$clients} ) {
          for my $c ( @{ $clients->{$client} } ) {
            ...
            if ($seq) {
              $self->app->log->debug(
                "Found valid return sequence... sending data");
      ...
      # monitor action
        ...
        if ($host) {
          ...
          for my $mon_itm ( @{$data} ) {    # iterate over all monitoring items
            ...
            for my $itm_name ( keys %{ $mon_itm->{values} } )
            {    # iterate over all values inside a monitoring item

              if (
                my ($counter) =
                grep { $_->{check_key} eq $itm_name } @counters
                )
              {

                $self->app->log->info( "found monitor for "
                    . $itm_name
                    . " on host "
                    . $host->name );
  ...
  #my $failure = $pcv->template_item->failure;
                ...
                $redis->publish( $self->config->{redis}->{monitor}->{queue} =>
                    Mojo::JSON->encode($redis_data) );
              }
              else {
                $self->app->log->error( "NO monitor found for "
                    . $itm_name
                    . " on host "
                    . $host->name );
              }
...
# update online status, and set expire so that if the host went offline we realize it after a specific timeout
        ...
        $redis->expireat(
          "status:$client_ip:online",
          time + (
            $self->config->{agent}->{keep_alive_timeout} *
              $self->config->{agent}->{keep_alive_flap}
          )
        );
        ...
        $self->send(
          Mojo::JSON->encode(
            {
              type        => "ping_answer",
              ping_answer => "Welcome to the real world.",
              interval    => $self->config->{agent}->{keep_alive_timeout}
            }
          )
        );
      ...
      # log action
        ...
        $redis->publish(
          $self->config->{redis}->{log}->{queue} => Mojo::JSON->encode($json) );
      }
