...
sub boot {
  ...
  my $redis =
    Mojo::Redis->new( server => $self->config->{redis}->{deploy}->{server} . ":"
      . $self->config->{redis}->{deploy}->{port} );
  ...
  my $mac = $self->dhcp->get_mac_for($client);
...
#$hw = Rex::IO::Server::Model::Hardware->all( Rex::IO::Server::Model::NetworkAdapter->ip == ip_to_int($client) );
  ...
  if ( my $system = $hw ) {
    ...
    if ( my $boot_os = $boot_os_r ) {
      ...
      elsif ( $self->param("finished") )
      {    # hook after installation, must be called from within the template

        $self->app->log->debug(
          "Installation finished, setting system state_id = 4 (client: $client)"
        );
      ...
      elsif ( $self->param("deploy") )
      {    # first boot after service os / after registration
            # return os template to deploy os

        if ( $boot_os->id == 1 ) {
          $self->app->log->debug(
            "Booting local... Setting system->state_id = 4");
          $system->update(
            {
              state_id => 4
            }
        ...
        if ( $boot_os->ipxe ) {
          ...
          $self->app->log->debug(
            "Booting ipxe commands... Setting system->state_id = 4");
          $system->update(
            {
              state_id => 4
            }
...
#my $boot_eth = Rex::IO::Server::Model::NetworkAdapter->all( Rex::IO::Server::Model::NetworkAdapter->mac eq $mac )->next;
        my $boot_eth =
          $self->db->resultset("NetworkAdapter")->search( { mac => $mac } )
          ->first;
        my $eth = $boot_eth->dev;
      ...
      elsif ( $self->param("kickstart") )
      {    # request of kickstart/preseed/... file
        ...
        $self->app->log->debug(
          "boot from local hard disk... state == INSTALLED");
...
#my $system_r = Rex::IO::Server::Model::Hardware->all( Rex::IO::Server::Model::Hardware->name eq $self->stash("server") );
  my $system_r = $self->db->resultset("Hardware")
    ->search( { name => $self->stash("server") } );
...
#my $os_r = Rex::IO::Server::Model::OsTemplate->all( Rex::IO::Server::Model::OsTemplate->name eq $self->stash("os") );
    my $os_r = $self->db->resultset("OsTemplate")
      ->search( { name => $self->stash("os") } );
    if ( my $os = $os_r->first ) {
      $system->update(
        {
      ...
      return $self->render(
        json   => { ok => Mojo::JSON->false, error => "OS not found" },
        status => 404
      );
    }
  }
  ...
  $self->render(
    json   => { ok => Mojo::JSON->false, error => "Host not found" },
    status => 404
  );
}
...
sub __register__ {
  ...
  $r->post("/deploy/:server/:os")->over( authenticated => 1 )
    ->to("deploy#deploy");

  $r->put("/deploy/os/:id")->over( authenticated => 1 )->to("deploy-os#update");
  $r->post("/deploy/os/:name")->over( authenticated => 1 )
    ->to("deploy-os#register");
  $r->delete("/deploy/os/:name")->over( authenticated => 1 )
    ->to("deploy-os#delete");
  $r->route("/deploy/os")->via("LIST")->over( authenticated => 1 )
    ->to("deploy-os#list");
}
