...
sub render {
  ...
  $self->app->log->debug( "shared data plugin_hooks from BaseController:\n"
  ...
  if ( $self->authenticated ) {
    $self->app->log->debug(
      "User permissions from BaseController: (" . $self->current_user() . ")" );
    @permissions =
    ...
    && exists $current_hooks->{ $config->{plugin} }->{ $config->{url} } )
  {
    ...
    for my $plugin ( @{$plugins} ) {
      ...
      if ( exists $plugin->{location} ) {
        ...
        my $resp =
          $ua->post( $backend_url =>
            { "Accept" => "application/json", %{$req_headers} } =>
            json => \@rest );
