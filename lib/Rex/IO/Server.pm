...
has shared_config => sub {
    my ($self) = @_;
    state $rd ||=
      Redis::DistLock->new( servers => [ $self->config->{redis}->{server} ] );
};
...
sub startup {
    ...
    $self->helper(
        shared_data_tx => sub {
            my ( $self, $code ) = @_;
            my $mutex =
              $self->shared_config->lock( "rexio_server_shared_config",
                $self->config->{redis}->{lock_ttl} || 10 );
            $code->();
    ...
    $self->helper(
        shared_data => sub {
            ...
            if ($value) {
                $self->redis->set(
                    "/rexio/server/$key" => Mojo::JSON::encode_json($value) );
            }
            else {
                if ($key) {
                    my $ret;
                    eval {
                        $ret =
                          JSON::XS::decode_json(
                            $self->redis->get("/rexio/server/$key") );
                        1;
                    } or do {
                        $ret = {};
                    };
        ...
        register_plugin => sub {
            ...
            $self->shared_data_tx(
                sub {
                    ...
                    my $current_plugin_config =
                      $self->shared_data("plugin_config");
                    $current_plugin_config->{$plugin_name} = $ref;
                    $self->shared_data(
                        "plugin_config" => $current_plugin_config );

                    for my $plugin_name ( keys %plugin_hooks ) {
                        for my $plugin_action (
                            keys %{ $plugin_hooks{$plugin_name} } )
                        {
                            push @{ $current_hooks->{$plugin_name}
                                  ->{$plugin_action} },
                    ...
                    $self->shared_data( "loaded_plugins",
                        \%merged_loaded_plugins );
                }
            );
        },
    );
...
#$r->post("/1.0/plugin/plugin")->over( authenticated => 1 )->to("plugin#register");
    ...
    for my $ref ( values %{$plugins} ) {
        $self->log->debug(
            "Loading already registered plugin: " . $ref->{name} );
